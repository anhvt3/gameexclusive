...
function checkApDocSize() {
  const apPattern = join(REPO_ROOT, 'docs', 'architecturepack_*.md');
  const docs = [];
  // sync glob (Node 22+ supports node:fs/promises glob)
  try {
    const cmd = process.platform === 'win32'
      ? `dir /b "${apPattern.replace(/\//g, '\\')}"`
      : `ls ${apPattern} 2>/dev/null`;
    const output = execSync(cmd, { stdio: ['pipe', 'pipe', 'ignore'] }).toString();
    output.split('\n').filter(Boolean).forEach((f) => docs.push(f.trim()));
  } catch {
    // no AP files — skip
  }
  ...
  for (const doc of docs) {
    ...
    if (size > 20 * 1024) {
      results.push({
        check: `AP size <= 20kb`,
        status: 'FAIL',
        details: `${relative(REPO_ROOT, doc)} is ${(size / 1024).toFixed(1)}kb — tách appendix theo rule #6`,
      });
    }
  }
...
function checkClevaiTerminology() {
  const CLEVAI_TERMS = [
    'DY1', 'DY2', 'DU', 'XH', 'KEN', 'KMA', 'HRG', 'ULC',
    'KEN_TT_DU', 'L4E', 'CUIE', 'USI', 'TEP', 'CHPT', 'POSUP', 'ARCH',
  ];

  const SRC_DIR = join(REPO_ROOT, 'app', 'src');
  if (!existsSync(SRC_DIR)) {
    results.push({ check: 'Clevai term no-hallucination', status: 'WARN', details: 'app/src/ not found, skipping' });
    return;
  }
      ...
      for (const term of CLEVAI_TERMS) {
        ...
        if (regex.test(line) && !/wiki:|wiki\//.test(line)) {
          ...
          if (!window.some((l) => /wiki:|wiki\//.test(l))) {
            violations.push(`${relative(REPO_ROOT, file)}:${i + 1} uses "${term}" without wiki: reference`);
          }
        }
      }
    }
...
function checkClaudeMdHarness() {
  ...
  const target = existsSync(gameClaudeFile) ? gameClaudeFile : existsSync(claudeFile) ? claudeFile : null;

  if (!target) {
    results.push({ check: 'CLAUDE.md Harness Principles', status: 'WARN', details: 'CLAUDE.md not found' });
    return;
  }
...
function checkNoSecrets() {
  ...
  for (const pat of FORBIDDEN) {
    try {
      const out = execSync(`git ls-files "${pat}"`, { cwd: REPO_ROOT, stdio: ['pipe', 'pipe', 'ignore'] })
        .toString()
        .trim();
